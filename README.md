...
Le champ `imageNight` donne au portrait une **variante de nuit** : la bascule se fait en CSS sur `[data-night='1']`, l'attribut que le moteur de scroll pose déjà sur le conteneur sticky. Aucun state React n'est en jeu, donc l'image change en direct si la fenêtre reste ouverte pendant la tombée de la nuit. La classe `--day` n'est posée que s'il existe une variante : une image seule n'est donc jamais masquée par la règle de nuit. À noter — la palette de la fenêtre a une transition de 0,55 s alors que l'image bascule d'un coup ; c'est visible si on cherche à le voir.
